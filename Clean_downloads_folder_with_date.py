...
parser.add_argument(
    ...
    default=[],
    nargs="+",
    help="type in extensions to filter in format py Default filters everything",
)
...
if config["extensions"] == "all":
    split_types = list(
        set(os.path.splitext(file)[-1] for file in os.listdir(os.curdir))
    )
else:
            ...
            if os.path.isfile(file):
                ...
                if not os.path.exists(str(file_time.strftime("%Y-%m-%d"))):
                    ...
                    if not os.path.exists(
                        str(file_time.strftime("%Y-%m-%d")) + "/" + file_ext_name
                    ):
                        os.mkdir(
                            str(file_time.strftime("%Y-%m-%d")) + "/" + file_ext_name
                        )
                shutil.move(
                    file, str(file_time.strftime("%Y-%m-%d")) + "/" + file_ext_name
                )
            else:
                for file in os.listdir(os.curdir):
                    if file.endswith(tuple(split_types)):
